...
func (d *driver) assurePrivateAccount(cfg *Azure, infra *configv1.Infrastructure, tagset map[string]*string, accountName string) (string, error) {
	if d.Config.NetworkAccess == "" || d.Config.NetworkAccess == imageregistryv1.AzureNetworkAccessExternal {
	...
	pe, err := azclient.CreatePrivateEndpoint(
		...
		&azureclient.PrivateEndpointCreateOptions{
			Location:            cfg.Region,
			VNetName:            d.Config.VNetName,
			SubnetName:          d.Config.SubnetName,
	...
	if err := azclient.ConfigurePrivateDNS(d.Context, pe, d.Config.VNetName, accountName); err != nil {
...
func (d *driver) RemoveStorage(cr *imageregistryv1.Config) (retry bool, err error) {
	...
	if d.Config.PrivateEndpointName != "" {
		...
		if err := azclient.DeletePrivateEndpoint(
			d.Context, d.Config.PrivateEndpointName,
		); err != nil {
			respErr, ok := err.(*azcore.ResponseError)
			if !ok || respErr.StatusCode != http.StatusNotFound {
				util.UpdateCondition(
