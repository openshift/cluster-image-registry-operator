...
const (
	...
	nativeSecretName = "image-registry-private-configuration-user"
)
...
func (m MockSecretNamespaceLister) Get(name string) (*corev1.Secret, error) {
	if name == nativeSecretName {
		return &corev1.Secret{
			Data: fakeSecretData,
		}, nil
	}
	if name == cloudSecretName {
		return &corev1.Secret{
			Data: fakeCloudsYAML,
		}, nil
	}

	return nil, fmt.Errorf("No secret with name %v was found", name)
}
...
func TestSwiftCreateStorageNativeSecret(t *testing.T) {
	...
	d := driver{
		Listers: &regopclient.Listers{
			Secrets:          MockSecretNamespaceLister{},
			InstallerSecrets: MockSecretNamespaceLister{},
		},
		Config: &config,
	}
...
func TestSwiftConfigEnvNativeSecret(t *testing.T) {
	config := imageregistryv1.ImageRegistryConfigStorageSwift{
...
func TestSwiftConfigEnvCloudConfig(t *testing.T) {
  ...
  ` + cloudName + `:
	...
	th.AssertEquals(t, "", res[6].Value)
	th.AssertEquals(t, "REGISTRY_STORAGE_SWIFT_USERNAME", res[7].Name)
	th.AssertEquals(t, username, res[7].Value)
	th.AssertEquals(t, false, res[7].ValueFrom != nil)
	th.AssertEquals(t, "REGISTRY_STORAGE_SWIFT_PASSWORD", res[8].Name)
	th.AssertEquals(t, password, res[8].Value)
	th.AssertEquals(t, false, res[8].ValueFrom != nil)
}
