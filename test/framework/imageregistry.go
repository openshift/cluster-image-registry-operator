...
func removeImageRegistry(te TestEnv) {
	...
	if operatorDeployment.Spec.Replicas != nil && *operatorDeployment.Spec.Replicas == 0 {
		...
		if !conds.Removed.IsTrue() {
			te.Fatalf("unable to uninstall the image registry: the operator is shutted down, but the image registry is not removed: %s, conditions: %v", config.Spec.ManagementState, conds)
		}
		return
	}
