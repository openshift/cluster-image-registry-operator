...
var _ = g.Describe("[sig-imageregistry][Serial] AWS", func() {
	g.It("TestAWSDefaults", func() {
		testAWSDefaults(g.GinkgoTB())
	})

	g.It("TestAWSUnableToCreateBucketOnStartup", func() {
		testAWSUnableToCreateBucketOnStartup(g.GinkgoTB())
	})

	g.It("TestAWSUpdateCredentials", func() {
		testAWSUpdateCredentials(g.GinkgoTB())
	})

	g.It("TestAWSChangeS3Encryption", func() {
		testAWSChangeS3Encryption(g.GinkgoTB())
	})

	g.It("TestAWSFinalizerDeleteS3Bucket", func() {
...
func testAWSDefaults(t testing.TB) {
	...
	if err != nil {
		t.Fatalf("unable to get secret %s/%s: %#v", defaults.ImageRegistryOperatorNamespace, defaults.ImageRegistryPrivateConfiguration, err)
	}
	...
	if err != nil {
		t.Fatalf("unable to create new session with supplied AWS credentials: %#v", err)
	}
	...
	if err != nil {
		t.Fatalf("unable to get public access block information for s3 bucket: %#v", err)
	}

	if getPublicAccessBlockResult == nil || getPublicAccessBlockResult.PublicAccessBlockConfiguration == nil {
		t.Fatalf("public access block configuration is nil")
	}

	gp := getPublicAccessBlockResult.PublicAccessBlockConfiguration

	if gp.BlockPublicAcls == nil || !*gp.BlockPublicAcls {
		if gp.BlockPublicAcls == nil {
			t.Errorf("PublicAccessBlock.BlockPublicAcls should have been true, but was nil instead")
		} else {
	...
	if gp.BlockPublicPolicy == nil || !*gp.BlockPublicPolicy {
		if gp.BlockPublicPolicy == nil {
			t.Errorf("PublicAccessBlock.BlockPublicPolicy should have been true, but was nil instead")
		} else {
	...
	if gp.IgnorePublicAcls == nil || !*gp.IgnorePublicAcls {
		if gp.IgnorePublicAcls == nil {
			t.Errorf("PublicAccessBlock.IgnorePublicAcls should have been true, but was nil instead")
		} else {
	...
	if gp.RestrictPublicBuckets == nil || !*gp.RestrictPublicBuckets {
		if gp.RestrictPublicBuckets == nil {
			t.Errorf("PublicAccessBlock.RestrictPublicBuckets should have been true, but was nil instead")
		} else {
	...
	if err != nil {
		t.Fatalf("unable to get tagging information for s3 bucket: %#v", err)
	}
	...
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			t.Fatalf("unable to get lifecycle information for S3 bucket: %#v, %#v", aerr.Code(), aerr.Error())
		} else {
			t.Fatalf("unknown error occurred getting lifecycle information for S3 bucket: %#v", err)
		}
	}
...
func testAWSUnableToCreateBucketOnStartup(t testing.TB) {
	...
	// Ensure the invalid credentials secret is always cleaned up
	defer func() {
		...
		if err != nil {
			t.Logf("unable to remove %s/%s: %#v", defaults.ImageRegistryOperatorNamespace, defaults.ImageRegistryPrivateConfigurationUser, err)
		}
	}()
...
func testAWSChangeS3Encryption(t testing.TB) {
	...
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			t.Fatalf("unable to get encryption information for S3 bucket: %#v, %#v", aerr.Code(), aerr.Error())
		} else {
			t.Fatalf("unknown error occurred getting encryption information for S3 bucket: %#v", err)
		}
	}
	...
	if _, err = te.Client().Configs().Patch(
		...
		t.Fatalf("unable to patch image registry custom resource: %#v", err)
	}
