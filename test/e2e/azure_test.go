...
func TestAzurePrivateStorageAccount(t *testing.T) {
	...
	patch := fmt.Sprintf(
		`{"spec": {"storage": {"azure": {"networkAccess": "%s", "vnetName": "%s", "subnetName": "%s"}}}}`,
		imageregistryv1.AzureNetworkAccessInternal, vnetName, subnetName,
	)
	...
	err = wait.PollUntilContextTimeout(ctx, time.Second, framework.AsyncOperationTimeout, true,
		func(ctx context.Context) (stop bool, err error) {
			...
			if cr.Spec.Storage.Azure.NetworkAccess == imageregistryv1.AzureNetworkAccessExternal {
	...
	exists, err := azclient.PrivateEndpointExists(ctx, azureConfig.PrivateEndpointName)
	if err != nil {
